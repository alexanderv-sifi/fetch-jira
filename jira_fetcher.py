...
JIRA_DEFAULT_PROJECT_KEY = os.getenv("JIRA_DEFAULT_PROJECT_KEY", "YOUR_JIRA_PROJECT") # Added for template
...
def fetch_jira_issue(issue_key):
    ...
    logging.debug(f"Fetching Jira issue: {url} with params: {params}")
...
def search_jira_with_jql(jql_query, context_log_prefix="  "):
    ...
    while True:
        ...
        logging.info(f"{context_log_prefix}Fetching JQL page: startAt={start_at}, maxResults={MAX_RESULTS_PER_JIRA_PAGE} for JQL: {jql_query[:50]}... with params {params}")
        JIRA_SEMAPHORE.acquire()
        try:
            time.sleep(API_CALL_DELAY_SECONDS)
            response = requests.get(url, headers=headers, auth=auth, params=params)
...
def fetch_epic_children(epic_key):
    ...
    jql = f'(parent = "{epic_key}" OR "Epic Link" = "{epic_key}") AND project = {JIRA_DEFAULT_PROJECT_KEY}'
...
def main():
    ...
    args = parser.parse_args() # Parse args inside main

    # Moved debug logging and GDrive init here, using 'args'
    if args.debug:
...
if __name__ == "__main__":
    ...
    # parser = argparse.ArgumentParser(description="Fetch Jira data based on issue, JQL, or project.") # Moved
    # parser.add_argument("--mode", choices=["issue", "jql", "project"], required=True, help="Mode to fetch Jira data: specific issue, JQL query, or entire project.") # Moved
    # parser.add_argument("--query", required=True, help="The Jira issue key, JQL query string, or Project key.") # Moved
    # parser.add_argument("--skip-remote-content", action="store_true", help="Skip fetching content from Confluence and Google Drive links.") # Moved
    # parser.add_argument("--debug", action="store_true", help="Enable debug logging.") # Moved
    
    # cli_args = parser.parse_args() # Moved

    # if cli_args.debug: # Moved
    #     logging.getLogger().setLevel(logging.DEBUG) # Moved
    #     logging.debug("Debug logging enabled.") # Moved

    # if GOOGLE_LIBS_AVAILABLE: # Moved
    #     get_google_drive_service() # Moved
    #     if not DRIVE_SERVICE: # Moved
    #         logging.warning("Failed to initialize Google Drive Service. GDrive fetching will be skipped.") # Moved

    main() # Simplified call
